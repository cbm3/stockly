...
export const getDashboard = async (): Promise<DashboardDto> => {
  ...
  const endtOfDay = new Date(new Date().setHours(23, 59, 59, 999));
  const totalRevenuePromise = db.$queryRawUnsafe<{ totalRevenue: number }[]>(totalRevenueQuery);
  const todayRevenuePromise = db.$queryRawUnsafe<{ todayRevenue: number }[]>(totalRevenueQuery, startOfDay, endtOfDay);
